...
function Eyebrow({ children, light = false }) {
  return <span className={`block text-[10px] font-semibold uppercase tracking-[0.42em] mb-4 ${light ? 'text-[#C8102E]' : 'text-[#C8102E]'}`} style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>{children}</span>;
}
...
export default function BlogPage() {
  ...
  return (
    <main id="main-content" className="bg-[#0B0B0B]">
        ...
        <div className="wrap relative z-10">
          <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.1, ease: [0.22, 1, 0.36, 1], delay: 0.3 }}>
            ...
            <h1 className="font-display text-[clamp(2.8rem,6vw,7rem)] font-medium leading-[0.95] tracking-tight text-[#F8F6F2] max-w-3xl" style={{ fontFamily: 'Cormorant Garamond, Georgia, serif' }}>
              Ideas on Living<br />
              <em style={{ color: '#C8102E', fontStyle: 'italic' }}>With Intention</em>
            </h1>
      ...
      <section className="bg-[#0B0B0B] py-20 border-t border-[#F8F6F2]/5">
        <div className="wrap">
          <Reveal className="mb-6">
            <span className="text-[10px] font-semibold uppercase tracking-[0.42em] text-[#C8102E]" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>Featured Essay</span>
          </Reveal>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            ...
            <Reveal delay={0.15} className="lg:py-8">
              <div className="flex items-center gap-4 mb-6">
                <span className="text-[10px] font-semibold uppercase tracking-[0.38em] text-[#C8102E]" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>{FEATURED_ARTICLE.category}</span>
              ...
              <div className="mt-8 flex items-center gap-3 text-[11px] font-medium uppercase tracking-[0.3em] text-[#C8102E] cursor-pointer hover:gap-5 transition-all duration-300" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
          ...
          <div className="flex flex-wrap gap-3">
            {TOPICS.map(topic => (
              <button
                ...
                className={`px-4 py-2 text-[10px] font-medium uppercase tracking-[0.3em] border transition-all duration-400 ${activeFilter === topic ? 'border-[#C8102E] bg-[#C8102E]/10 text-[#C8102E]' : 'border-[#F8F6F2]/10 text-[#A0A0A0] hover:border-[#C8102E]/40 hover:text-[#F8F6F2]'}`}
              ...
              <Reveal key={article.id} delay={i * 0.07}>
                <article className="group cursor-pointer">
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    <span className="text-[10px] font-semibold uppercase tracking-[0.38em] text-[#C8102E]" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>{article.category}</span>
                  ...
                  <h3 className="font-display text-xl md:text-2xl font-medium leading-[1.2] tracking-tight text-[#F8F6F2] mb-3 group-hover:text-[#C8102E] transition-colors duration-400" style={{ fontFamily: 'Cormorant Garamond, Georgia, serif' }}>
                    {article.title}
                  </h3>
                  <p className="text-sm text-[#A0A0A0] font-light leading-[1.75]" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
                    {article.excerpt}
                  </p>
                  <div className="mt-5 h-px w-0 group-hover:w-16 bg-[#C8102E] transition-all duration-500" />
                </article>
              </Reveal>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <Reveal>
              ...
              <h2 className="font-display text-[clamp(2rem,4vw,4rem)] font-medium leading-[1.05] tracking-tight text-[#F8F6F2]" style={{ fontFamily: 'Cormorant Garamond, Georgia, serif' }}>
                ...
                <em style={{ color: '#C8102E', fontStyle: 'italic' }}>Design & Living Well</em>
              </h2>
            ...
            <Reveal delay={0.15}>
              ...
              <div className="flex flex-col sm:flex-row gap-0">
                <input
                  ...
                  className="flex-1 bg-transparent border border-[#F8F6F2]/15 px-5 py-3.5 text-sm text-[#F8F6F2] placeholder-[#A0A0A0]/50 focus:outline-none focus:border-[#C8102E]/50 transition-colors duration-300"
                  style={{ fontFamily: 'Inter, system-ui, sans-serif' }}
                />
                <button className="bg-[#C8102E] text-[#0B0B0B] px-7 py-3.5 text-[11px] font-semibold uppercase tracking-[0.3em] hover:bg-[#F8F6F2] transition-colors duration-400 whitespace-nowrap" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
                  Subscribe
                </button>
              </div>
      ...
      {/* ══ 6. CTA ══ */}
      <section className="relative bg-[#FAF6F0] py-24 overflow-hidden border-t border-[#0B0B0B]/10">
        <div className="wrap relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
          <Reveal>
            <h2 className="font-display text-[clamp(2.2rem,4vw,4rem)] font-medium leading-[1.0] tracking-tight text-[#111111]" style={{ fontFamily: 'Cormorant Garamond, Georgia, serif' }}>
              Ready to Build Your<br /><em style={{ color: '#C8102E', fontStyle: 'italic' }}>Dream Space?</em>
            </h2>
          </Reveal>
