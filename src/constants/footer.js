import { IconFacebook, IconInstagram, IconLinkedIn } from '../components/ui/FooterIcons';

export const FOOTER_STUDIOS = [
  { city: 'Chennai Experience Center', region: 'No. 3, Seemathamman Nagar, Maduravoyal' },
];

export const FOOTER_COLLECTIONS = [
  'Timeless Tiles',
  'Exquisite Granites',
  'Custom Furniture',
];

export const FOOTER_SOCIAL = [
  { label: 'Instagram', href: 'https://instagram.com', icon: IconInstagram },
  { label: 'LinkedIn', href: 'https://www.linkedin.com/company/amaraliving', icon: IconLinkedIn },
  { label: 'Facebook', href: 'https://facebook.com', icon: IconFacebook },
];

